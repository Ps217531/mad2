...
const TicTacToe = () => {
    ...
    const onPress = (rowIndex, columIndex) => {
        ...
        setMap((existingMap) => {
            const updateMap = [...existingMap];
            let options = [];
            for (let row in updateMap) {
                for (let col in updateMap[row]) {
                    if (updateMap[row][col] === "") {
                        options.push([row, col]);
                    }    
                }
            }
            let winner = getWinner();
            if (winner) return existingMap;
            if (options.length <= 0) return existingMap;
            let random = Math.floor(Math.random() * options.length);
            updateMap[options[random][0]][options[random][1]] = "o";
            return updateMap;
        });
        setCurrentTurn("x");
    };
